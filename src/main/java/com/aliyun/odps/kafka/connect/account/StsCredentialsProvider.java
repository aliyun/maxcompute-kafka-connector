...
import static com.aliyun.odps.kafka.connect.ConfigParameter.ACCOUNT_ID;
import static com.aliyun.odps.kafka.connect.ConfigParameter.REGION_ID;
import static com.aliyun.odps.kafka.connect.ConfigParameter.ROLE_NAME;
import static com.aliyun.odps.kafka.connect.ConfigParameter.STS_ENDPOINT;
...
public class StsCredentialsProvider implements ICredentialProvider {
    ...
    public StsCredentialsProvider(ConnectorConfig config) {
        this.config = config;
    }
    ...
    private void fetchAndCacheNewCredentials() throws CredentialException {
        try {
            String ak = ACCESS_ID.getString(config);
            String sk = ACCESS_KEY.getString(config);
            ...
            if (ak == null || ak.isEmpty() || sk == null || sk.isEmpty()) {
                throw new CredentialException(
                    "Cannot get Aliyun Access Key ID/Secret from config or environment variables.");
            }

            String accountId = ACCOUNT_ID.getString(config);
            String regionId = REGION_ID.getString(config);
            String roleName = ROLE_NAME.getString(config);
            String stsEndpoint = STS_ENDPOINT.getString(config);
    ...
    @Override
    public void close() {
        log.info("Closing STS credentials provider");
    }
}
